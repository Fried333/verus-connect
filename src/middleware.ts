...
const results = new Map<string, { iAddress: string; friendlyName: string; extra?: Record<string, unknown>; consumed: boolean }>();
...
export function verusAuth(config: VerusAuthConfig) {
  ...
  router.get('/result/:challengeId', (req: any, res: any) => {
    ...
    // One-time consumption: return the result once, then clean up.
    // Prevents replay attacks where an attacker re-reads the result
    // (which may contain JWTs or session tokens from the onLogin hook).
    if (result.consumed) {
    ...
    // Schedule cleanup — give a small grace window for network retries,
    // then delete so the data doesn't sit in memory.
    setTimeout(() => {
      challenges.delete(challengeId);
      results.delete(challengeId);
    }, 10_000);
