...
Drop-in VerusID login and payments for any website. One package — standalone server, Express middleware, and frontend SDK.
...
| `/auth/verus/generic-request` | POST | Create a GenericRequest deep link (v3) |
| `/auth/verus/identity-update-request` | POST | Create an identity update request deep link (v3) |
...
Generate a deep link that opens Verus Mobile with a pre-filled payment request. Supports R-addresses, i-addresses, and VerusID names:

```javascript
// Pay to a VerusID — wallet shows the identity name
const res = await fetch('/auth/verus/pay-deeplink', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    address: 'Verus Coin Foundation@',  // VerusID name, i-address, or R-address
    amount: 1.5,                         // Amount in VRSC
    currency_id: 'i5w5MuNik5NtLcYmNzcvaoixooEebB6MGV', // Optional, defaults to VRSC
  }),
});
const { deep_link, destination_type, resolved_address } = await res.json();
// destination_type: "identity" or "address"
// resolved_address: the i-address or R-address used
```

**Smart address detection (v3):**

| Input | Destination Type | Wallet Shows |
|-------|-----------------|--------------|
| `Verus Coin Foundation@` | `DEST_ID` | Identity name |
| `i5v3h9FWVdRFbNHU7DfcpGykQjRaHtMqu7` | `DEST_ID` | Identity name |
| `RAddress...` | `DEST_PKH` | R-address |

Identity names are automatically resolved to i-addresses via the configured RPC endpoint.
...
Request a user to update their VerusID content multimap. The wallet prompts the user to approve the update.

```javascript
// POST /auth/verus/identity-update-request
const res = await fetch('/auth/verus/identity-update-request', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    identity: 'veruscx@',
    updates: {
      contentMultiMap: {
        'iGTUp2Tnd2eYAih717dyFHzbokhy6DgrH2': { message: 'Hello from my app' }
      }
    }
  }),
});
...
> **Note:** Requires Verus Mobile with GenericRequest support (in development). The endpoint generates valid deep links now — the wallet will process them once the `generic-request-changes` branch is merged.

### Generic Request

Create an arbitrary request using the GenericRequest envelope. Supports any OrdinalVDXFObject detail type.

```javascript
// POST /auth/verus/generic-request
const res = await fetch('/auth/verus/generic-request', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    details: [{ vdxfkey: 'iGTUp2Tnd2eYAih717dyFHzbokhy6DgrH2', data: 'test' }]
  }),
});
const { deep_link, qr_string, has_callback } = await res.json();
```

The callback URL is automatically set to the server's configured callback endpoint. It cannot be overridden by the client (security: prevents redirect attacks).
...
verus-connect v3 includes built-in rate limiting on the `/login` endpoint (10 challenges per minute per IP). For additional protection, add rate limiting at the reverse proxy level:
...
| `/pay-deeplink` | POST | Generate a VerusPay invoice deep link (supports name@, i-address, R-address) |
| `/generic-request` | POST | Create a GenericRequest deep link with arbitrary details |
| `/identity-update-request` | POST | Create an identity update request deep link |
| `/health` | GET | Health check (includes `genericRequestSupported` flag) |
