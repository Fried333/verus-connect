...
**v1** shipped as middleware-only — every developer had to:
1. Write their own Express server (`index.js` boilerplate)
2. Manually install `verusid-ts-client` as a peer dependency
3. Figure out PM2/systemd configuration
4. Debug module resolution issues (PM2 uses a different `require.main`, causing `verusid-ts-client` to not be found)
5. Set up CORS, JSON parsing, graceful shutdown

**v2** adds a **standalone server mode** that eliminates all of that:

| | v1 | v2 |
|---|---|---|
| **Setup** | Write Express server + install peer deps | `npm install` + `.env` + `verus-connect start` |
| **Module resolution** | `callerRequire` from entry script (breaks with PM2) | `smartRequire` tries cwd first, then entry script (works everywhere) |
| **Dependencies** | `express` and `verusid-ts-client` are peer deps | `express`, `cors`, `dotenv` bundled; only `verusid-ts-client` is peer |
| **CLI** | None | `verus-connect start` with `--port`, `--host`, `--env`, `--cors` flags |
| **PM2 support** | Manual, often broken | Works out of the box, includes `ecosystem.config.cjs` example |
| **CORS** | DIY | Built into standalone server, configurable via env |
| **Backwards compat** | — | Supports legacy env names (`SERVER_URL`, `API`) |

### Breaking Changes

None. The middleware API (`verusAuth()`) is unchanged. If you're using v1 with your own Express server, it still works exactly the same — just with better module resolution.

### Upgrading

If you have an existing sidecar with a custom `index.js`:

```bash
# Update the package
npm install verus-connect@latest

# Option 1: Keep your custom index.js (it still works)
pm2 restart verus-sidecar

# Option 2: Switch to the CLI (delete your index.js)
pm2 delete verus-sidecar
pm2 start npx --name verus-sidecar -- verus-connect start
```

The CLI reads the same `.env` file your custom server was using. If you used `SERVER_URL` instead of `CALLBACK_URL`, it still works — the CLI auto-converts `SERVER_URL` to `CALLBACK_URL` by appending `/verusidlogin`.
